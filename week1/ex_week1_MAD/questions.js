...
function rotateArray(array, numRotations) {
  const totalCycles = numRotations % array.length;
...
function animalClassify(type, weight) {
  const size = weight < 5 ? "small" : weight < 100 ? "big" : "huge";
  return { type, size };
}
...
function describePerson(person) {
  ...
  const gender = isFemale(person.gender) ? "woman" : "man";
  const pronoun = isFemale(person.gender) ? "She" : "He";

  if(person.hobby){
    return `${person.name} is a ${person.age}-year-old ${gender}. ${pronoun} likes ${person.hobby}!`
  }else{
    return `${person.name} is a ${person.age}-year-old ${gender}. ${pronoun} has no hobbies.`
  }
}
...
function analyzeMarks(marks) {
  ...
  const lowestMark = Math.min(...markValues);
  const highestMark = Math.max(...markValues);
  const averageMark = markValues.reduce((sum, mark) => sum + mark, 0) / markValues.length;
  return [lowestMark, highestMark, Number(averageMark.toFixed(2))];
}
