...
export const checkWinner = (movement) => {
    ...
    for (let i = 0; i < win.length; i++) {
        if (movement.filter(item => win[i].includes(item.index) && item.player === "X").length === 3) return "X";
        if (movement.filter(item => win[i].includes(item.index) && item.player === "O").length === 3) return "O";
        if (movement.length === 9) return "Draw";
    }
    return "";
}
